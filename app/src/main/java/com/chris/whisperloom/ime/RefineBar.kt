 ...
 * Stufen als ausklappbare Zeile ueber der Tastenreihe. Nach dem Muster von [GestureTargets]
 * eine Klasse um die Views, damit der Dienst nur verdrahtet.
 *
 * Eingeklappt ist die Zeile `GONE` — die Tastatur darf im Ruhezustand nicht hoeher werden.
 *
 * Ohne eingerichteten KI-Zugang sind die drei oberen Stufen abgeblendet statt ins Leere zu
...
class RefineBar(
    ...
    /**
     * Zeigt die Zeile mit [selected] als gewaehlter Stufe. [llmReady] `false` blendet die drei
     * KI-Stufen ab.
     */
    fun show(selected: RefineMode, llmReady: Boolean) {
        select(selected)
